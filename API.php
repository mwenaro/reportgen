...
//    private $req_data;
    ...
    function dump($var, $str = null) {
        echo json_encode(['var' => $var, 'where' => $str]);
    }
...
//        var_dump($req_string);
        ...
        $url1 = array_shift($req_string_array);
//        $url_options = !empty($req_string_array) ? array_shift($req_string_array) : null;

        /**
         * check if is rest api
         */
        $this->dump($url1, 'b4 api checkpoint');
        if (strpos(strtolower($url1), 'api')) {
            $this->is_restapi = true;
            $url = substr($url, 3);
        }
        $this->dump($url, 'b4 api checkpoint');
        /**
         * check if is an end points api
         */
        if (strpos(strtolower($url), 'endpoints')) {
            $this->getEndPoints = true;
            $url = substr(trim($url, '/'), 9);
        }
...
//        echo json_encode([
//            'con_met' => $con_method,
//            'con_met1' => $this->controller_method,
//            'reg_met' => $this->request_method,
//            'url_array' => $url_array,
//        ]);
