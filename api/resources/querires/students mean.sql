SELECT DISTINCT adm,name,form,exam_type,term,year,SUM(score) AS tMarks,COUNT(sub) AS subjects,SUM(points) AS total_points ,ROUND(AVG(points),3) AS mp FROM(
...
) GROUP BY adm,examId,form
ORDER BY examId,form,mp  DESC