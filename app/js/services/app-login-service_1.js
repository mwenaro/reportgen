    ...
    function (h, $location, $state, paths, sessionService, $rootScope) {
      ...
      return {
        loginUser: function (user) {
          var validate = h.post(api_login + "login", {
            request: "login",
            user: user,
          });
        ...
        isLoggedIn: function () {
          var checkSession = h.post(api_login + "isLoggedIn", {
            request: "check_login",
          });
        ...
        valid: {
          valPhone: function (phone1) {
            ...
            return (
              (/^07/.test(phone) && phone.length === 10) ||
              (/^[+254]/.test(phone) && phone.length === 13) ||
              (/^[254]/.test(phone) && phone.length === 12)
            );
          },
          goToUrl: function () {
